...
public class OpenApiDocumentationCustomizer {
    ...
    private static void customizeOperation(String path, PathItem.HttpMethod method, Operation operation) {
        ...
        switch (method.name() + " " + path) {
            ...
            case "POST /api/servicos" -> createAutomotiveService(operation);
            case "PUT /api/servicos/{id}" -> updateAutomotiveService(operation);
            case "GET /api/servicos" -> listAutomotiveServices(operation);
            case "GET /api/servicos/{id}" -> getAutomotiveService(operation);
            case "PATCH /api/servicos/{id}/inativar" -> inactivateAutomotiveService(operation);
            case "PATCH /api/servicos/{id}/ativar" -> activateAutomotiveService(operation);
            case "POST /api/itens" -> createInventoryItem(operation);
            case "PUT /api/itens/{id}" -> updateInventoryItem(operation);
            case "GET /api/itens" -> listInventoryItems(operation);
            case "GET /api/itens/{id}" -> getInventoryItem(operation);
            case "PATCH /api/itens/{id}/inativar" -> inactivateInventoryItem(operation);
            case "PATCH /api/itens/{id}/ativar" -> activateInventoryItem(operation);
            case "POST /api/estoque/entrada" -> registerInventoryEntry(operation);
            case "POST /api/estoque/baixa" -> registerInventoryWithdrawal(operation);
            case "POST /api/estoque/ajuste" -> registerInventoryAdjustment(operation);
            case "GET /api/estoque/itens/{itemId}/movimentacoes" -> listInventoryMovements(operation);
    ...
    private static String resolveTag(String path) {
        ...
        if (path.startsWith("/api/servicos")) {
